...
class Tuner(object):
    ...
    def tune(self, n_trial, measure_option, early_stopping=None, callbacks=(), si_prefix="G",time_budget=1e9):
        """Begin tuning
        ...
        time_budget: int
            The time budget for this tuning. Stop the tuning when time budget runs out.
            
        Returns
        -------
        res_no: int
            Type of stop this tuning: 
            0 for running out of n_trial;
            1 for running out of time_budget;
            2 for early_stopping triggerred;
        """
        start_ti = time.time()
        ...
        error_ct = 0
        res_no = 0
        while self.iters < n_trial:
            if not self.has_next():
                break
            if time.time()-start_ti>=time_budget:
                res_no = 1
                break
            configs = self.next_batch(min(n_parallel, n_trial - self.iters))
            ...
            self.iters += len(results)
            # self.ttl = min(early_stopping + self.best_iter, n_trial) - i

            self.update(inputs, results)
            for callback in callbacks:
                callback(self, inputs, results)
            if self.iters >= self.best_iter + early_stopping:
