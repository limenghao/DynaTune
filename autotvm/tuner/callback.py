...
def progress_bar(total, prefix="", si_prefix="G"):
    ...
    def _callback(tuner, inputs, results):
        ...
        for inp, res in zip(inputs, results):
            if res.error_no == 0:
                flops = inp.task.flop / np.mean(res.costs)
        # if not logger.isEnabledFor(logging.DEBUG):  # only print progress bar in non-debug 
...
def update_task_state(ctx):
    """Update TaskState object for each task
    
    """
    def _callback(tuner, inputs, results):
        ctx.iters += len(inputs)
        ctx.xs = [x for x in range(1, ctx.iters+1)]
        for k, (inp, res) in enumerate(zip(inputs, results)):
            if res.error_no == 0:
                flops = inp.task.flop / np.mean(res.costs)
            else:
                flops = 0
            ctx.flops_max = max(ctx.flops_max, flops)
            ctx.ys.append(float(ctx.flops_max) / 1e9)
        
    return _callback

def dyn_progress_bar(ctx, si_prefix="G"):
    """Display progress bar for tuning

    Parameters
    ----------
    ctx: TaskState object, include all context of this tuning task
    si_prefix: str
        SI prefix for flops
    """
    tic = time.time()
        ...
        sys.stdout.write(
            ...
            "| %.2f s" %
             (
                    ctx.prefix,
                    format_si_prefix(0, si_prefix),
                    format_si_prefix(ctx.flops_max, si_prefix),
                    si_prefix,
                    ctx.iters,
                    ctx.n_trial, # len(ctx.task.config_space),
                    time.time() - tic + ctx.time_cost,
                )
        )
        sys.stdout.flush()
