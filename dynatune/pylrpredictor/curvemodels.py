...
class CurveModel(object):
    ...
    def predict_given_theta(self, x, theta):
        """
            Make predictions given a singlef theta
        """
...
class MCMCCurveModel(CurveModel):
    ...
    def __init__(self,
        ...
        self.exceeding_constraint = exceeding_constraint
    ...
    def ln_prior(self, theta, x, y):
        ...
        if self.are_params_in_bounds(theta):
            ...
            # print(theta, self.are_params_in_bounds(theta))
            return -np.inf
    ...
    def ln_prob(self, theta, x, y):
        ...
        """
        lp = self.ln_prior(theta,x,y)
        if not np.isfinite(lp):
            return -np.inf
        return lp + self.ln_likelihood(theta, x, y)
    
    def plotprocess(self,sampler):
        import matplotlib.pyplot as plt
        import matplotlib
        matplotlib.rcParams.update({'font.size': 20})
        fig, axes = plt.subplots(3, figsize=(10, 3), sharex=True)
        samples = sampler.get_chain()
        print(samples)
        print(samples.shape)
        # labels = ["c", "alpha", "sigma"]
        labels = ["a",'b','c','sigma']
        for i in range(3):
            ax = axes[i]
        ...
        axes[-1].set_xlabel("Step number", fontsize=24)
        plt.subplots_adjust(left=0.05, right=0.97, top=0.98, bottom=0.1)
        plt.show()
    ...
    def fit_mcmc(self, x, y):
        ...
        # print("ml learned params")
        # print(list(self.ml_curve_model.ml_params))
        ...
        # print("accept fractions:")
        # print(list(sampler.acceptance_fraction) + list(self.ml_curve_model.ml_params))
        # print(sum(sampler.acceptance_fraction)/len(sampler.acceptance_fraction))
        # self.plotprocess(sampler)
    
    def plot_theta(self, data):
        import matplotlib.pyplot as plt
    ...
    def predict(self, x):
        """
            E[f(x)]
        """
        predictions = self.predictive_distribution(x, thin=10)
